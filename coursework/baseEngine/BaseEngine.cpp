...
*/ 
int BaseEngine::Initialise( char const* strCaption, int iScreenWidth, int iScreenHeight, char const* szFontName, int iFontSize )
{
							...
							unsigned int uiColour, Font* pFont, SDL_Surface* pTarget )
{
	...
	SDL_Color color = { static_cast<Uint8>((uiColour&0xff0000)>>16), static_cast<Uint8>((uiColour&0xff00)>>8), static_cast<Uint8>((uiColour&0xff)), 0 };

	if ( ( pFont != NULL ) && ( pFont->GetFont() != NULL ) )
	{
		SDL_Surface *sText = TTF_RenderText_Solid( pFont->GetFont(), pText, color );
		SDL_Rect rcDest = {static_cast<Sint16>(iX),static_cast<Sint16>(iY),0,0};
